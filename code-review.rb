...
def process_application_created_event(application)
    ...
    fund_accounts.each do |fund_account|
      if fund_account.match_location?(application.full_address, application.latlong)
         #  COMMENT REVIEW 1  allocate_funding_to_application is not in the FundAccount class
        allocate_funding_to_application(application, fund_account)
         #  COMMENT REVIEW 2 can also return success message on allocate_funding_to_application
        return
      end
    end
  ...
  class FundAccount
     #  COMMENT REVIEW 3 Why was location_match commented out since it was reference in criteria_location_matching_priority to evaluete the switch case
    def location_match
    ...
    #  COMMENT REVIEW 4 thoes   GeoService.distance_between return a boolean
    ...
    private def matches_location_by_polygon?(location_match, latlong)
      return false if latlong.nil?
   #  COMMENT REVIEW 5 thoes   GeoService.point_inside_polygon return a boolean
